#' To identify report for data, report name may have 'reports/' prefix
#'
#' @param report Report name with possible 'reports/' prefix
#'
#' @return Report name without prefix
...
#'
#' @return tibble of report details
#' @export
get_bo_report_details <- function(conn, document, report = '') {
  ...
  if (!is_null_or_empty(report)) {
    ...
    reports %>% dplyr::filter(.data$name == report_name)
  } else {
...
get_bo_report_element_data <- function(conn, document, report, element) {
  logger::log_info("Getting report","{report}", "for document", "{document}", ";t get_bo_report_element_data 33")
  document_id = get_bo_item_id(document)
  report_details <- get_bo_report_details(conn, document, report)
  report_id <- report_details$id
  elements <- request_bo_raylight_endpoint(conn = conn, documents = document_id, reports = report_id, elements = '')
  elements <- elements %>% dplyr::filter(.data$id == element | .data$name == element)
  element_id <- elements$id
  data <- request_bo_raylight_endpoint(conn, documents = document_id, reports = report_id, elements = element_id, dataset = '')
  ...
  logger::log_info(paste("Report data retrieved", "{report_details$name}", "for document", "{document$SI_NAME}"))
  result
}
