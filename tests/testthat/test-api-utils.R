test_that(paste("Item", Sys.getenv('BO_TEST_DOCUMENT_NAME'), "is found"), {
  ...
  d1 <- get_bo_item(conn, name = Sys.getenv('BO_TEST_DOCUMENT_NAME'), parent_folder = get_test_folder_id(conn), owner = NULL, kind = "Webi")
  print (nrow(d1))
  # browser()
...
test_that(paste("Multiple items are found in a folder"), {
  conn <- open_bo_connection(server=Sys.getenv('BO_TEST_SERVER'))
  d2 <- get_bo_item(conn, parent_folder = get_test_folder_id(conn))
  expect_gt(nrow(d2), 1)
})

test_that(paste("Multiple items are found in a folder matching type 'Excel'"), {
  conn <- open_bo_connection(server=Sys.getenv('BO_TEST_SERVER'))
  d3 <- get_bo_item(conn, parent_folder = get_test_folder_id(conn), kind='Excel')
  expect_gt(nrow(d3), 1)
})
